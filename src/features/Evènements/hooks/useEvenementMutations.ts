...
import { createEvenement, updateEvenement, deleteEvenement } from '@/features/Evènements/api/Evenements.api';
...
export function useUpdateEvenement() {
  ...
  return useMutation({
    ...
    onSuccess: (evenementMisAJour) => {
      queryClient.invalidateQueries({ queryKey: ['evenements'] });
      queryClient.invalidateQueries({ queryKey: ['evenements', evenementMisAJour.id] });
    },
  });
}
